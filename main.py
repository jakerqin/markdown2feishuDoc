...
def sync_target(target, args, sync_state, get_feishu_client):
    target_id = target.id
    markdown_dir = normalize_config_path(target.local_dir)
    root_folder_token = target.feishu_folder_token

    print(f"开始同步目标: {target_id}")
    print(f"本地目录: {markdown_dir}")
    ...
    if args.mark_current_synced:
        for file_info in markdown_files:
            sync_state.mark_uploaded(file_info, target_id=target_id)
        sync_state.save()
        print(f"已将{len(markdown_files)}个Markdown文件标记为已同步，不执行上传")
        return {"uploaded": 0, "skipped": len(markdown_files)}

    changed_files = [
        file_info
        for file_info in markdown_files
        if sync_state.has_changed(file_info, target_id=target_id)
    ]
    ...
    if not changed_files:
        ...
        return {"uploaded": 0, "skipped": skipped_count}
    ...
    folder_mapping.update({
        ...
        for path, token in sync_state.get_folders(target_id).items()
    })
    ...
    for file_info in changed_files:
        ...
        if folder_path:
            ...
            for part in folder_parts:
                ...
                if current_path in folder_mapping:
                    ...
                    folder_token = sync_state.get_folder_token(current_path, target_id=target_id)
                    ...
                    sync_state.mark_folder(current_path, folder_token, target_id=target_id)
                    sync_state.save()

            parent_token = folder_mapping[current_path]

        # 上传markdown文件为飞书文档（有历史映射则覆盖旧文档）
        existing_doc_token = sync_state.get_doc_token(file_info, target_id=target_id)
        ...
        sync_state.mark_uploaded(file_info, doc_token=doc_token, target_id=target_id)
        sync_state.save()
        uploaded_count += 1

        print(f"  文档上传完成: {file_name}")

    print(f"目标 {target_id} 同步完成：上传{uploaded_count}个，跳过{skipped_count}个")
...
def main():
    ...
    if (
        not os.getenv('FEISHU_APP_ID')
        or not os.getenv('FEISHU_APP_SECRET')
        or os.getenv('FEISHU_APP_ID') == 'your_feishu_app_id'
        or os.getenv('FEISHU_APP_SECRET') == 'your_feishu_app_secret'
    ):
    ...
    try:
        print(f"开始从本地Markdown文件同步到飞书，共{len(targets)}个同步目标")
        ...
        for target in targets:
            try:
                result = sync_target(target, args, sync_state, get_feishu_client)
                total_uploaded += result["uploaded"]
                total_skipped += result["skipped"]
            except Exception as e:
                failed_count += 1
                print(f"目标 {target.id} 同步失败: {str(e)}")

        print(f"同步完成：上传{total_uploaded}个，跳过{total_skipped}个，失败目标{failed_count}个")
